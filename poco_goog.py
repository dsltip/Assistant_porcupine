        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(),
            ...
            if len(resp.audio_out.audio_data) > 0:
                if not self.conversation_stream.playing:
                    ...
                    logging.info('Playing assistant response.')			#									!!!!!!!!!!!!!!Play response
                    logging.info('My user request: "%s" %f',mytranscript,mystability)
                    if "сколько" in mytranscript.lower():
                        #Do your voice local actions here 
                        logging.info('Найдено "сколько", завершаемся')
                        self.conversation_stream.stop_playback()
                        return False
                self.conversation_stream.write(resp.audio_out.audio_data)			# occurs many times while answering
